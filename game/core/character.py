...
class TestowaWalka():
    def __init__(self):
        self.gracz=Player()
        self.wrog=Enemy()
    def CompareOponents(self):
        print(self.gracz.__str__())
        print(self.wrog.__str__())
    def Fight(self):
        while(self.gracz.current_hp!=0 or self.wrog.current_hp!=0):
            self.gracz.current_hp=self.gracz.current_hp-self.wrog.mele_attack
            self.wrog.current_hp=self.wrog.current_hp-self.gracz.mele_attack
            print(self.CompareOponents())
        
if __name__ == "__main__":
    walka=TestowaWalka()
    walka.CompareOponents()
    walka.Fight()
